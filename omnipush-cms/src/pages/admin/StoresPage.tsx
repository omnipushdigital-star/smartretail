...
import { Plus, Search, Edit2, Trash2, Store as StoreIcon, Loader2 } from 'lucide-react'
...
export default function StoresPage() {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            if (editing) {
                const { error } = await supabase.from('stores').update({ ...form, updated_at: new Date().toISOString() }).eq('id', editing.id)
                if (error) throw error
                toast.success('Store updated')
            } else {
                const { error } = await supabase.from('stores').insert({ ...form, tenant_id: DEFAULT_TENANT_ID })
                if (error) throw error
        ...
        <div>
            <div className="page-header">
                <div>
                    ...
                    <p className="page-subtitle">Manage your retail store locations</p>
                </div>
                        ...
                        <div className="table-wrapper">
                            <table>
                                ...
                                <tbody>
                                    {paginated.map(s => (
                                        <tr key={s.id}>
                                            <td><span style={{ fontFamily: 'monospace', fontWeight: 500 }}>{s.code}</span></td>
                                            ...
                                            <td>
                                                <div style={{ display: 'flex', gap: '0.5rem' }}>
                                                    ...
                                                    <button onClick={() => handleDelete(s.id)} className="btn-danger" style={{ padding: '0.375rem 0.625rem' }} disabled={deleting === s.id}>
                    ...
                    <form onSubmit={handleSave}>
                        <div className="form-row">
                            <div className="form-group">
                                ...
                                <input id="store-code" className="input-field" value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value }))} placeholder="e.g. STORE001" disabled={!!editing} />
                            </div>
                            <div className="form-group">
                                <label className="label">Store Name *</label>
                                <input id="store-name" className="input-field" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Downtown Branch" />
                            </div>
                        </div>
                        <div className="form-group">
