...
// ΓöÇΓöÇΓöÇ Unified Double-Buffer Engine ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function UnifiedDoubleBuffer({ items, assets, idx, onAdvance, effect = 'fade', showDebug = false, isNative = false, consecutiveErrorsRef, lastMediaErrorRef }: {
    ...
    // ΓöÇΓöÇ Slot State ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // Stable mutable refs ΓÇö never stale in callbacks
    ...
    const getItemData = useCallback((item: ManifestItem): { url: string; type: string } => {
        ...
        if (!url && !item.media_id && item.type !== 'web_url') {
            console.warn(`[UDB] ΓÜá∩╕Å Item ${item.playlist_item_id} has no URL or media_id. Fallback to placeholder.`);
        }
    ...
    // ΓöÇΓöÇ Watchdog ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    const advanceBuffer = useCallback((forceNext = false) => {
        ...
        const killDecoder = (v: HTMLVideoElement | null) => {
            ...
            try {
                ...
                v.load()
                console.log('[UDB] ≡ƒÆÇ Hardware decoder purged.')
            } catch (e) {
    ...
    // ΓöÇΓöÇ Boot ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    useEffect(() => {
    ...
    // ΓöÇΓöÇ Stall Monitor ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    useEffect(() => {
    ...
    // ΓöÇΓöÇ Slot Styles ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                    ...
                    <div key={i} style={slotStyle}>
                        {type === 'video' ? (
                            <video
                                ...
                                // stopping ΓåÆ existing watchdog naturally fires. Adding onStalled
    ...
    useEffect(() => {
        ...
        if (filtered.length > 0) {
            if (nextIds !== lastActiveIdsRef.current) {
                console.log(`[PlaybackEngine] ≡ƒôï Active items updated: ${filtered.length} items`)
            ...
            console.warn(`[PlaybackEngine] ΓÜá∩╕Å Current manifest empty, but using cached stable playlist (${prevActiveItemsRef.current.length} items) to avoid black screen.`)
...
export default function PlayerPage() {
    ...
    // ΓöÇΓöÇΓöÇ BOOT-CRITICAL: DO NOT CHANGE 90000 ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // activates ΓÇö causing an infinite reload cycle on boxes without network at boot.
    ...
    const fetchManifest = useCallback(async (sec: string, timeoutMs: number = 25000): Promise<boolean> => {
        try {
            ...
            // ΓöÇΓöÇ Auto version-change detection or first load ΓöÇΓöÇ
            if (newVersion && newVersion !== versionRef.current) {
                console.log(`[Player] ≡ƒöä New version/content detected: ${versionRef.current || 'init'} ΓåÆ ${newVersion}`)
            }
            ...
            // ΓöÇΓöÇΓöÇ BOOT-CRITICAL: DO NOT REMOVE backoff OR failCountRef >= 3 check ΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            // failCountRef >= 3 is the trigger for offline cache load ΓÇö this is how the device
    ...
    const sendHeartbeat = useCallback(async (sec: string) => {
        ...
        // ΓöÇΓöÇ Rendering Watchdog ΓöÇΓöÇ
    ...
    useEffect(() => {
        ...
        if (stored) {
            ...
            // DO NOT add waitForNetwork() ΓÇö navigator.onLine is broken on Chromium 87.
    ...
    useEffect(() => {
        ...
        // ΓöÇΓöÇΓöÇ STAGGERED HEARTBEAT FIX ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Pre-calculate corner tap zone ΓöÇΓöÇ
    const cornerTapZone = (
        <div
            onClick={(e) => {
                e.stopPropagation();
                ...
                <div id="admin-overlay" style={{
                    ...
                    <div style={{ padding: '1.5rem 2rem', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
                        {[
                            ...
                            { label: 'Regions', value: Object.keys(manifest?.region_playlists || {}).join(', ') || 'ΓÇö' },
    ...
    // ΓöÇΓöÇ Main Content Resolver ΓöÇΓöÇ
    ...
    const renderMain = () => {
        ...
        // ΓöÇΓöÇ Standby / No Content published yet ΓöÇΓöÇ
